...
popups <- paste0(
  ...
  # Spanish version
  '<div class="bi-mix">',
  '<b style="font-size:14px;">', "Non kuu ja ni sa'a ja ni na saama", '</b><br>',
...
lang_btn <- tags$button(
  ...
  tags$div(class = "bi-mix", "\U0001F310  Tu’un Savi")
)
...
legend_ctrl <- tags$div(
  ...
  tags$b(tags$div(class = "bi-mix", "Ni na saama")),
  tagList(swatch_rows)
)
...
# Layers names
group_labels <- c(
  "Esri.WorldTopoMap" = "<span class='bi-es'>ESRI Topografía</span><span class='bi-mix'>ESRI Topografía</span>",
